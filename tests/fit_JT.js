...
// collect per rung: moves with weighted points
const rungs = [];
...
const crossed = rows.some(r => r.ratio >= 1);
const trend = rows.length >= 2 ? (rows[rows.length - 1].ratio - rows[0].ratio) : 0;
console.log('\nVERDICT: ' + (crossed
  ? 'the cooling protocol CROSSES the Curie point — spontaneous initiative polarization reached.'
  : `subcritical at every rung; ratio ${trend > 0.02 ? 'RISES' : trend < -0.02 ? 'falls' : 'is flat'} as the bath cools ` +
    `(${rows.map(r => r.ratio.toFixed(2)).join(' → ')}).`));

// ── SVG: criticality ratio vs ⟨T⟩ with the Curie line ──
const W = 760, H = 460, PAD = 60;
const xs = rows.map(r => r.meanT);
const xmin = Math.min(...xs) - 0.3, xmax = Math.max(...xs) + 0.3;
const ymax = Math.max(1.2, ...rows.map(r => r.ratio * r.ci.J[1] / Math.max(r.J, 1e-9))) + 0.1;
...
svg += `<text x="${W / 2}" y="24" fill="#e0e0e0" text-anchor="middle" font-size="17">Cooling scan: criticality ratio J·b/2T vs bath temperature</text>`;
svg += `<text x="${W / 2}" y="42" fill="#888" text-anchor="middle" font-size="12">self-play, four thinking-time rungs · precision-weighted Curie–Weiss refit per rung · Curie point at ratio = 1</text>`;
...
for (const r of rows) svg += `<text x="${X(r.meanT)}" y="${H - PAD + 16}" fill="#666" font-size="10" text-anchor="middle">${r.meanT.toFixed(2)}</text>` +
  `<text x="${X(r.meanT)}" y="${H - PAD + 30}" fill="#557" font-size="9" text-anchor="middle">${r.ms}ms</text>`;
svg += `<text x="${W / 2}" y="${H - 10}" fill="#888" font-size="12" text-anchor="middle">⟨T⟩ (bath, per rung) — colder to the left</text>`;
let d = '';
for (let i = 0; i < rows.length; i++) d += (i ? 'L' : 'M') + X(rows[i].meanT).toFixed(1) + ',' + Y(rows[i].ratio).toFixed(1);

...
| `gge_infodecomp.js` | **READ 1 of the GGE charge-cohort pre-registration** (`docs/gge_charge_cohort.html` §9). The evaluation F = ⟨Q⟩ + T·S is the r=1 projection of the tree ensemble; this measures the incremental cross-validated information ΔI_k of each candidate charge — capacity Var_π (seat 1), basin occupations (seat 2), the T_eff-proxy √Var/T (seat 3), the even modulator lnW₊ (seat 4) — about the off-horizon verdict Y = e(d4)−e(d2) (same-parity revision), *beyond* the seated {⟨Q⟩, S}. Method: closed-form hat-matrix ridge LOO-CV R², permutation null, partial Spearman. Kill criterion (pre-registered): all ΔR²_k within the null AND \|partial ρ\| < 0.15 ⇒ the physical cohort is empty and no seat is instantiated. Corpus = exposure catastrophes + quiet random controls; deep is node-capped (read as a lower bound on ΔI). `node gge_infodecomp.js [DEEP] [NRAND] [SEED]`. **VERDICT — CORRECTED (see `gge_infodecomp_robust.js`):** the raw-value ridge returned a KILL (every ΔR²<0, C_slow −1.19), but that was a **heavy-tail artifact** — the capacity charges span 0→160 and their standardised outliers blow up the closed-form LOO. Redone in **rank/normal-score space** (tail-robust): the **r=2 capacity charge C is a genuine candidate** (ΔR²(rank) +0.049, p≈0.06, partial-ρ +0.33) and the **cohort clears p<0.05 jointly** (ΔR² +0.099, p 0.03, jackknife >0 in 42/42). Coherent with read 2 (the variance *response* was the robust survivor). Necessary condition met, not the gauntlet. Raw feature rows persisted in the JSON |
| `gge_infodecomp_robust.js` | **the tail-robust redo of read 1** (offline, from the persisted feature rows — no engine re-run). Diagnoses the raw-ridge KILL as a heavy-tail leverage artifact (C_slow ΔR² −1.19 is impossible for an uninformative feature) and re-runs the whole decomposition (marginal, joint, pairwise interactions, permutation null, partial correlation, drop-one jackknife) in rank/normal-score space. VERDICT: the r=2 capacity charge C is a seat candidate; the cohort is jointly significant (p 0.03). `node gge_infodecomp_robust.js [SEED]` |
| `gge_seat1_derive.js` | **LEG 1** — derive the seat-1 multiplier from measurement, not tuning (offline). The reshaping shifts Φ by ≈ −b₂·Var/T, so requiring that to equal the measured off-horizon revision Y≈β·Var gives **b₂ = −β̂·T**. Measures β̂ robustly (Theil–Sen + rank, controlling ⟨Q⟩,S) and — decisively — the SIGN: ρ(Var,Y\|⟨Q⟩,S) = **+0.24, STABLE across strata** (catastrophes +0.31, controls +0.44) ⇒ F under-values spread ⇒ **b₂<0**, derived β̂=0.047. `node gge_seat1_derive.js` |
| `depth_discriminator.js` | **the routing question**: can a measured SHALLOW signal flag "this cold value needs 8-ply depth" better than σ_eff (which is blind at the catastrophes)? Contrast confident-and-WRONG (exposure catastrophes) vs eval-matched confident-and-RIGHT (recovered confident wins); AUC per signal with bootstrap CI. **VERDICT:** σ_eff / gap / drift BLIND (CI straddles 0.5); β (initiative) REFUTED (saturated −1 in both — the forcing census can't route); the ONE robust flag is **oppOpt = lnW_them−lnW_us, the danger sense** (AUC 0.70, CI [0.59,0.82], catastrophes higher), with Ceff (capacity) a weaker second (AUC 0.33, cat↓). Read honestly: oppOpt is the leaf term F ALREADY prices — too weak as a term (exposure: bounded ~1♙ vs a −∞ mate) but usable as an ALLOCATION flag (route ln N / selective depth to high-oppOpt nodes, rule-3 clean). The fresh take on the failed freeze-guard, which gated on the blind value-flux. `node depth_discriminator.js` |
| `vs_stockfish.js gge` (`results/vs_sf1500_gge.json`) | **LEG 3** — the sufficiency test: gauntlet the DERIVED seat-1 evaluator (per-node b₂ = −0.047·T, stability-guarded) vs SF-1500. **VERDICT: 5.5/12 vs the 7.5 baseline (W5 D1 L6), mate losses 3→6, bath 16.6→6.2 — NEGATIVE.** Seat 1 carries real ΔI (read 1) but pricing it LOSES: rewarding spread steers a weak tactician into sharp positions it can't convert (Goodhart — the static variance→improvement correlation doesn't survive being made an objective). The Gumbel meta-law again; the frame's win was LOCATING the r=2 sector, not pricing it |
